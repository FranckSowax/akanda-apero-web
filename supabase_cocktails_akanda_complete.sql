...
('Verres à Cocktail', 'Set de verres adaptés selon les cocktails choisis', '🥃', 3000, 2),
('Décoration Fruits', 'Fruits frais pour décoration et garniture', '🍓', 2500, 3),
('Glaçons Extra', 'Glaçons supplémentaires et glace pilée', '🧊', 1500, 4);

-- =====================================================
-- COCKTAILS ET MOCKTAILS CONSOLIDÉS
-- =====================================================

-- Ajout des cocktails consolidés
...
('Mojito', 'Le classique cocktail cubain à la menthe fraîche', '🌿', 2, 8, 2500, 'classique', 11.0, true, true),
('Piña Colada', 'Cocktail tropical crémeux à la noix de coco', '🥥', 2, 10, 2800, 'tropical', 12.0, true, true),
('Dark ''n'' Stormy', 'Cocktail épicé au rhum brun et ginger beer', '⛈️', 2, 5, 2600, 'classique', 14.0, true, false),
('Cosmopolitan', 'Cocktail élégant à la vodka et cranberry', '🍸', 3, 8, 3200, 'classique', 15.0, true, false),
('Margarita', 'Cocktail mexicain à la tequila et citron vert', '🍋', 2, 7, 2900, 'classique', 16.0, true, true),
('Mai Tai', 'Cocktail polynésien aux deux rhums', '🌺', 3, 10, 3500, 'tropical', 18.0, true, false),
('Tequila Sunrise', 'Cocktail coloré effet dégradé', '🌅', 2, 5, 2700, 'tropical', 14.0, true, false),
('Whisky Sour', 'Cocktail aigre-doux au whisky', '🥃', 3, 8, 3200, 'classique', 20.0, true, false),
('French 75', 'Cocktail pétillant au gin et champagne', '🥂', 3, 7, 3800, 'classique', 12.0, true, false),
('Espresso Martini', 'Cocktail énergisant au café', '☕', 4, 10, 3500, 'signature', 16.0, true, false),
('Ndoss Mix', 'Cocktail signature à la mangue', '🥭', 2, 8, 2800, 'signature', 12.0, true, true),
('Lambar Cocktail', 'Cocktail au corossol et menthe', '🍃', 2, 7, 2600, 'signature', 11.0, true, false),
('Okoumé Sunset', 'Gin à la pastèque et fruits rouges', '🍉', 3, 10, 3200, 'signature', 13.0, true, false),
('Bissap Breeze', 'Rhum à l''infusion de bissap', '🌺', 3, 12, 2900, 'signature', 12.0, true, false);

-- Ajout des mocktails consolidés
...
('Zébu-Fizz', 'Mélange pétillant ananas-orange', '🍍', 1, 5, 1500, 'tropical', true, true),
('Pink Banana', 'Smoothie rose à la banane et fraise', '🍌', 1, 7, 1800, 'fruité', true, true),
('Mango Tango', 'Cocktail sans alcool à la mangue', '🥭', 2, 8, 2000, 'tropical', true, false),
('CocoKids', 'Lait de coco et ananas pour enfants', '🥥', 1, 5, 1600, 'tropical', true, true);
...
-- =====================================================
-- VÉRIFICATION ET STATISTIQUES FINALES
-- =====================================================

-- Vérification du nombre d'enregistrements
SELECT 'cocktails_maison' as table_name, COUNT(*) as total_records FROM cocktails_maison
UNION ALL
SELECT 'cocktail_ingredients' as table_name, COUNT(*) as total_records FROM cocktail_ingredients
UNION ALL
SELECT 'cocktail_instructions' as table_name, COUNT(*) as total_records FROM cocktail_instructions
UNION ALL
SELECT 'mocktails' as table_name, COUNT(*) as total_records FROM mocktails
UNION ALL
SELECT 'mocktail_ingredients' as table_name, COUNT(*) as total_records FROM mocktail_ingredients
UNION ALL
SELECT 'cocktail_options' as table_name, COUNT(*) as total_records FROM cocktail_options;

-- Vérification des cocktails actifs par catégorie
...
-- Vérification des mocktails actifs
...
-- Message de succès
SELECT 'AKANDA APÉRO - Base de données cocktails créée avec succès!' as status;
