-- Script SQL pour le système de points de fidélité
...
-- 2. Créer une table pour l'historique des transactions de points si elle n'existe pas
-- Note: customer_id fait référence à customers.id qui est lié à auth.users.id
...
DO $$
BEGIN
  -- Vérifier si la contrainte customer_id existe déjà
  IF NOT EXISTS (
  ...
  -- Vérifier si la contrainte order_id existe déjà
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'loyalty_transactions_order_id_fkey'
    AND table_name = 'loyalty_transactions'
  ) THEN
    ALTER TABLE loyalty_transactions 
...
-- 3. Activer RLS sur la table loyalty_transactions
ALTER TABLE loyalty_transactions ENABLE ROW LEVEL SECURITY;

-- 4. Politiques RLS pour loyalty_transactions
...
-- 5. Fonction pour calculer les points selon la catégorie
...
-- 6. Fonction pour ajouter des points à un client
...
-- 7. Fonction pour utiliser des points (rédemption)
...
-- 8. Trigger pour attribuer automatiquement des points lors d'une nouvelle commande
...
BEGIN
  ...
  IF FOUND THEN
    ...
    PERFORM add_loyalty_points(
      ...
      'earn',
      order_category,
      'Points gagnés pour la commande #' || NEW.id,
      NEW.id
    );
  END IF;
...
-- 12. Exemples d'utilisation des fonctions

-- Ajouter des points manuellement (bonus anniversaire par exemple)
