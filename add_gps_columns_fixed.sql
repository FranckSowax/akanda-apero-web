...
-- Version corrigée qui s'adapte aux enums existants
...
-- 4. Créer une vue de statistiques adaptée aux valeurs enum existantes
DROP VIEW IF EXISTS order_statistics;

-- Créer la vue avec une logique adaptative pour les statuts
CREATE OR REPLACE VIEW order_statistics AS
SELECT 
    COUNT(*) as total_orders,
    -- Utiliser les valeurs de statut qui existent réellement
    COUNT(CASE WHEN status::text = 'delivered' OR status::text = 'Livrée' THEN 1 END) as delivered_orders,
    COUNT(CASE WHEN status::text IN ('pending', 'En cours', 'confirmed', 'preparing') THEN 1 END) as pending_orders,
    COUNT(CASE WHEN status::text = 'cancelled' OR status::text = 'Annulée' THEN 1 END) as cancelled_orders,
    -- Utiliser total_amount (ajoutée par le script)
    COALESCE(
        SUM(CASE WHEN status::text = 'delivered' OR status::text = 'Livrée' THEN 
            COALESCE(total_amount, 0) 
        ELSE 0 END), 0
    ) as total_revenue,
    COALESCE(
        AVG(CASE WHEN status::text = 'delivered' OR status::text = 'Livrée' THEN 
            COALESCE(total_amount, 0) 
        END), 0
...
BEGIN
    ...
    RAISE NOTICE '📊 Vue order_statistics mise à jour (adaptée aux enums existants)';
    RAISE NOTICE '';
    RAISE NOTICE '🧪 Vous pouvez maintenant tester l''API Orders !';
    RAISE NOTICE '💡 Utilisez check_enum_values.sql pour voir les valeurs de statut disponibles';
END $$;
