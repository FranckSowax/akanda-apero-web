-- Script de configuration admin pour ohmygab.marketplace@gmail.com (VERSION CORRIGÉE)
...
-- 2. Supprimer les fonctions existantes pour éviter les conflits
DROP FUNCTION IF EXISTS check_user_admin_role(UUID);
DROP FUNCTION IF EXISTS get_user_admin_role();

-- 3. Créer le type enum pour les rôles admin (si pas déjà créé)
DO $$ 
BEGIN
...
-- 4. Créer la table admin_profiles (si pas déjà créée)
...
-- 6. Activer RLS sur la table admin_profiles
ALTER TABLE admin_profiles ENABLE ROW LEVEL SECURITY;

-- 7. Supprimer les anciennes politiques RLS
...
-- 9. Créer la fonction pour vérifier le rôle admin d'un utilisateur
...
-- 10. Créer la fonction pour obtenir le rôle admin de l'utilisateur connecté
...
-- 11. Créer ou remplacer la fonction de mise à jour du timestamp
...
-- 12. Supprimer et recréer le trigger
...
-- 13. Insérer ou mettre à jour le profil admin pour ohmygab.marketplace@gmail.com
DO $$
DECLARE
    admin_user_id UUID;
BEGIN
